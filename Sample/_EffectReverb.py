...
class CreateReverb:
    def __init__(self,time_in_ms=1500):
        ...
        self.reverb_time_in_samples = numpy.int((time_in_ms/1000)*config.sampling_rate)
    ...
    class __CreateReverbDelayLine:
        def __init__(self, reverb_time_in_samples,time_in_ms=500, feedback_loops=2, lowcut_filter_frequency=40, highcut_filter_frequency=12000,
            ...
            self.time_in_samples = numpy.int(reverb_time_in_samples//feedback_loops)#numpy.int(time_in_ms * (config.sampling_rate / 1000))
    ...
    def __CreateWhitenoise():
        whitenoise_time_array = numpy.arange(config.chunk_size)
        freqs = numpy.abs(numpy.fft.fftfreq(config.chunk_size, 1 / config.sampling_rate))
        f = numpy.zeros(config.chunk_size)

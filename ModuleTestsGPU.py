...
"""
Module GPU Testing for pyaudiodsptools. Just $pip install pyAudioDspTools.
...
pyAudioDspTools.config.initialize(44100, 88200, use_gpu=True)
...
from pyAudioDspTools.EffectFFTFilterGPU import CreateHighCutFilterGPU, CreateLowCutFilterGPU
from pyAudioDspTools.EffectEQ3BandFFTGPU import CreateEQ3BandFFTGPU
...
sine_chunked = cupy.array(MakeChunks(sine_copy)) # Convert to cupy array
print('done.')
print('')

print('####Creating Effect Classes####')
lowcuttest = CreateLowCutFilterGPU(200)
highcuttest = CreateHighCutFilterGPU(8000)
eq3bandffttestgpu = CreateEQ3BandFFTGPU(100, 2, 700, -4, 8000, 5)
print('done...')
print('')
...
counter = 0
for counter in range(len(sine_chunked)):
    sine_chunked[counter] = eq3bandffttestgpu.apply(sine_chunked[counter])
    counter += 1
stop = time.perf_counter()
print('Success!')
print('Total Time: ', (stop - start) * 1000, 'ms')
